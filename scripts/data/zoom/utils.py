...
def crop_borders(image, percent=5):
    h, w = image.shape[:2]
...
def local_alignment(im_movings, im_target):
    ...
    for im_moving in im_movings:
        ...
        if len(accept) > MIN_MATCH_COUNT:
            ...
            hr_corners = cv2.perspectiveTransform(hr_corners, transform)

            hr_transform = cv2.getPerspectiveTransform(hr_corners.reshape(4, 2),
                                                       np.float32([[0, 0], [0, hr_height - 1], [hr_width - 1, hr_height - 1], [hr_width - 1, 0]]))

            im_reg = cv2.warpPerspective(im_moving, hr_transform, (hr_width, hr_height), cv2.INTER_LINEAR,
